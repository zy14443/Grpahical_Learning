...
K = 2 # total number of coins/arms/machines
...
n_1_Bernstein_graph = np.ones((theta_2.shape))
n_2_Bernstein_graph = np.ones((theta_2.shape))
epsilon_min_graph = np.zeros((theta_2.shape))
...
for count, theta_head in enumerate(theta_2):
    ...
    c_prime = N/((alpha_1+alpha_2)*np.log(2/delta))
    
    epsilon_min = (1+np.sqrt(1+18*c_prime*(alpha_1*sigma_1+alpha_2*sigma_2)/(alpha_1+alpha_2)))/(3*c_prime)   
    epsilon_min_graph[count] = epsilon_min
    ...
    if theta_1*sigma_1>sigma_2:
        ...
        n_2_Bernstein_graph[count] = np.log(2/delta)*(2*sigma_2+2/3*epsilon_min)/(epsilon_min*epsilon_min)-n_1_Bernstein_graph[count]*theta_1
...
for i in range(0,M):
    for count,theta_head in enumerate(theta_2):
        flip_series_1 = (np.random.rand(n_1_Bernstein_graph[count].astype(int)) <= theta_1).astype(int)
        flip_series_2 = (np.random.rand(n_2_Bernstein_graph[count].astype(int)) <= theta_head).astype(int)
        
        n1_temp = n_1_Bernstein_graph[count]
        n2_temp = n_2_Bernstein_graph[count]+sum(flip_series_1)
        
        error_1_Bernstein[count,i] = np.log(2/delta)*(1+np.sqrt(1+18*n1_temp*theta_1*(1-theta_1)/np.log(2/delta)))/(3*n1_temp)
        error_2_Bernstein[count,i] = np.log(2/delta)*(1+np.sqrt(1+18*n2_temp*theta_head*(1-theta_head)/np.log(2/delta)))/(3*n2_temp)
...
ax.plot(theta_2, epsilon_min_graph, label = 'epsilon_min_graph')
# ax.plot(theta_2, error_1_Bernstein_average, label = 'error_1_Bernstein_average')
ax.plot(theta_2, error_2_Bernstein_average, label = 'error_2_Bernstein_average')

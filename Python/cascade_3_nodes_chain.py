...
error_Bernstein_graph_chain = np.ones((theta_1.shape))
...
for count, theta_head in enumerate(theta_1):    
    ...
    error_Bernstein_graph_chain[count] = epsilon_min
    ...
    n_3_Bernstein_graph[count] = np.log(2/delta)*(2*sigma_3+2/3*epsilon_min)/(epsilon_min*epsilon_min)-n_2_Bernstein_graph[count]*theta_2-n_1_Bernstein_graph[count]*theta_head*theta_2
    
    
#%%
error_1_Bernstein = np.zeros((theta_1.shape[0],M))
error_2_Bernstein = np.zeros((theta_1.shape[0],M))
error_3_Bernstein = np.zeros((theta_1.shape[0],M))

for i in range(0,M):
    for count,theta_head in enumerate(theta_1):
        flip_series_1 = (np.random.rand(n_1_Bernstein_graph[count].astype(int)) <= theta_head).astype(int)
        flip_series_2 = (np.random.rand(n_2_Bernstein_graph[count].astype(int)+sum(flip_series_1)) <= theta_2).astype(int)
        flip_series_3 = (np.random.rand(n_2_Bernstein_graph[count].astype(int)) <= theta_3).astype(int)

        
        n1_temp = n_1_Bernstein_graph[count]
        n2_temp = n_2_Bernstein_graph[count]+sum(flip_series_1)
        n3_temp = n_3_Bernstein_graph[count]+sum(flip_series_2)
        
        error_1_Bernstein[count,i] = np.log(2/delta)*(1+np.sqrt(1+18*n1_temp*theta_head*(1-theta_head)/np.log(2/delta)))/(3*n1_temp)
        error_2_Bernstein[count,i] = np.log(2/delta)*(1+np.sqrt(1+18*n2_temp*theta_2*(1-theta_2)/np.log(2/delta)))/(3*n2_temp)
        error_3_Bernstein[count,i] = np.log(2/delta)*(1+np.sqrt(1+18*n3_temp*theta_3*(1-theta_3)/np.log(2/delta)))/(3*n3_temp)


error_1_Bernstein_average = np.average(error_1_Bernstein, axis=1)
error_2_Bernstein_average = np.average(error_2_Bernstein, axis=1)
error_3_Bernstein_average = np.average(error_3_Bernstein, axis=1)



fig, ax = plt.subplots()

ax.plot(theta_1, error_Bernstein_graph_chain, label = 'error_Bernstein_graph_chain')
ax.plot(theta_1, error_1_Bernstein_average, label = 'error_1_Bernstein_average')
ax.plot(theta_1, error_2_Bernstein_average, label = 'error_2_Bernstein_average')
ax.plot(theta_1, error_3_Bernstein_average, label = 'error_3_Bernstein_average')
...
n_1_Bayesian_prime = np.zeros((theta_1.shape[0],M))
n_2_Bayesian_prime = np.zeros((theta_1.shape[0],M))
n_3_Bayesian_prime = np.zeros((theta_1.shape[0],M))
...
for i in range(0,M):   
    ...
    for count, theta_head in enumerate(theta_1):
        ...
        n_1_Bayesian_prime[count,i] = a_1+b_1-2
        n_2_Bayesian_prime[count,i] = a_2+b_2-2
        n_3_Bayesian_prime[count,i] = a_3+b_3-2
...
#%%
error_1_Bayesian = np.zeros((theta_1.shape[0],M))
error_2_Bayesian = np.zeros((theta_1.shape[0],M))
error_3_Bayesian = np.zeros((theta_1.shape[0],M))

for i in range(0,M):
    for count,theta_head in enumerate(theta_1):                
        n1_temp = n_1_Bayesian_prime[count,i]
        n2_temp = n_2_Bayesian_prime[count,i]
        n3_temp = n_3_Bayesian_prime[count,i]
        
        error_1_Bayesian[count,i] = np.log(2/delta)*(1+np.sqrt(1+18*n1_temp*theta_head*(1-theta_head)/np.log(2/delta)))/(3*n1_temp)
        error_2_Bayesian[count,i] = np.log(2/delta)*(1+np.sqrt(1+18*n2_temp*theta_2*(1-theta_2)/np.log(2/delta)))/(3*n2_temp)
        error_3_Bayesian[count,i] = np.log(2/delta)*(1+np.sqrt(1+18*n3_temp*theta_3*(1-theta_3)/np.log(2/delta)))/(3*n3_temp)


error_1_Bayesian_average = np.average(error_1_Bayesian, axis=1)
error_2_Bayesian_average = np.average(error_2_Bayesian, axis=1)
error_3_Bayesian_average = np.average(error_3_Bayesian, axis=1)



fig, ax = plt.subplots()
ax.plot(theta_1, error_Bernstein_graph_chain, label = 'error_Bernstein_graph_chain')
ax.plot(theta_1, error_1_Bayesian_average, label = 'error_1_Bernstein_average')
ax.plot(theta_1, error_2_Bayesian_average, label = 'error_2_Bernstein_average')
ax.plot(theta_1, error_3_Bayesian_average, label = 'error_3_Bernstein_average')

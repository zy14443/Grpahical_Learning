...
while cost_sum_greedy<N_max
     ...
     psi_greedy = Q_greedy*(-M_constraint_hat*n_next);
     ...
     if (sum(sum(abs(theta_hat_matrix-theta_matrix)>ones(grid_m,grid_n).*epsilon))==0)%sum(Q_greedy)==0 sum(sum(cnt_matrix<reshape(b,grid_m,grid_n)))==0
         break;
     end
end
...
while cost_sum<N_max
     ...
     psi_off = Q_off*(-M_constraint_hat*n_next);
     ...
     cost_sum = cost_sum+1;
     Q_off = max(zeros(1,length(theta)),Q_off+(b-M_constraint_hat*n_off_total)'); %reshape(cnt_matrix,[],1)
